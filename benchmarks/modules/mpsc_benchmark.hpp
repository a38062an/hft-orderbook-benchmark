...
class MpscBenchmark
{
  public:
    ...
    static MpscResult run(const std::string& /*bookName*/,
                          std::unique_ptr<IOrderBook> book,
                          const std::vector<Order>& orders,
                          int producerCount,
                          size_t warmupCount = 500)
    {
        ...
        for (int p = 0; p < producerCount; ++p)
        {
            ...
            producers.emplace_back([&, from, to]()
            {
 ...
 */
inline void printMpscTable(const std::string& book,
                            const std::string& scenario,
                            const std::vector<MpscResult>& results)
{
    std::cout << "\n" << std::string(100, '=') << "\n";
    std::cout << "MPSC BENCHMARK — Book: " << book << "  Scenario: " << scenario << "\n";
    std::cout << std::string(100, '=') << "\n";
    std::cout << std::left  << std::setw(10) << "Producers"
              << std::right << std::setw(14) << "Que_Mean(ns)"
              << std::setw(14) << "Que_P99(ns)"
              << std::setw(14) << "Eng_Mean(ns)"
              << std::setw(14) << "Eng_P99(ns)"
              << std::setw(16) << "Throughput(k/s)"
              << std::setw(10) << "Dropped"
              << std::setw(12) << "PeakDepth"
              << "\n" << std::string(100, '-') << "\n";

    for (const auto& r : results)
    {
        std::cout << std::left  << std::setw(10) << r.producerCount
                  << std::right << std::fixed << std::setprecision(1)
                  << std::setw(14) << r.queueMeanNs
                  << std::setw(14) << r.queueP99Ns
                  << std::setw(14) << r.engineMeanNs
                  << std::setw(14) << r.engineP99Ns
                  << std::setw(16) << static_cast<uint64_t>(r.throughputOrdersPerSec / 1000)
                  << std::setw(10) << r.ordersDropped
