...
\draw [arrow] (fill_check) -| node[anchor=south east, xshift=-0.2cm] {No (Partial)} (partial);
\draw [arrow] (fill_check) -| node[anchor=south west, xshift=0.2cm] {Yes (Full)} (full);
\draw [arrow] (partial) |- (end);
\draw [arrow] (full) |- (end);

% Annotations
\node (prob) [text width=4cm, text centered, font=\small\itshape, right=0.5cm of fill_check] 
