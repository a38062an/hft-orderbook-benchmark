...
longer per replicate; this balances precision against total campaign time.
...
To quantify estimation uncertainty, we report 95\% confidence intervals using:
\[
\bar{x} \pm t_{0.975,\,n-1}\frac{s}{\sqrt{n}},
\]
where $\bar{x}$ is the sample mean, $s$ is sample standard deviation, and $n$
is sample size. The term $t_{0.975,\,n-1}$ is the 97.5th percentile of the
Student-$t$ distribution with $n-1$ degrees of freedom (the two-sided 95\%
critical value). For hypothesis interpretation, this gives a practical rule:
when two implementations are compared within the same scenario, non-overlapping
intervals indicate a stronger signal that the observed latency difference is
...
using contracts to reinforce the \textbf{Liskov Substitution Principle}~\citep{liskov1987}. 
Beyond validating correctness, this strengthens code maintainability. Utilizing the 
...
(summarized in Table~\ref{tab:code_coverage_results}), proving that the 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../plots/scenario_validity/mixed_profile_right.png}
...
This "quad-timestamp" strategy ensures the integrity of the end-to-end flow; 
...
the greatest direct throughput of 1,442,735~ops/s achieved by the
\textbf{ArrayOrderBook}. It remains clearly ahead of the next tier
(\textbf{PoolOrderBook} at 810,382~ops/s and \textbf{MapOrderBook} at
799,534~ops/s), while \textbf{VectorOrderBook} is lowest at 693,564~ops/s.
...
For these best-versus-worst direct comparisons, the 95\% intervals are clearly
separated in the generated summaries, so the performance gap is unlikely to be
explained by run-to-run noise alone.

To rigorously evaluate whether the measured latency differences are the result of 
structural design choices or merely random measurement noise, we perform 
formal pairwise comparisons using Welch's t-test \citep{welch1947generalization}.
This analysis tests the null hypothesis (H0): the assumption that there is no 
real performance difference between two implementations, and any observed gap 
is due to chance. To reject this hypothesis, we calculate a t-statistic, 
which interprets the "signal" (the difference in mean latency) relative to 
the "noise" (the variance or jitter in the data). A larger t-statistic indicates 
that the performance gap is many times larger than the underlying randomness 
of the system.

Welch's variant of the t-test is specifically selected because it does not assume 
equal variances between the compared groups. In low-latency software, different 
algorithms (e.g., array indexing vs. pointer-chasing in a pool) often exhibit 
different levels of "tail jitter," making this robust approach necessary for 
scientific accuracy \citep{welch1947generalization}.

Furthermore, because we are making four separate comparisons (comparing the 
Array implementation against the Map, Pool, Vector, and Hybrid books), 
we must account for the "multiple comparisons problem." If we use a standard 
5\% significance level ($\alpha = 0.05$) for each test, the cumulative probability 
of finding a "false positive" by pure luck increases across the four tests. 
To maintain a strict overall confidence level, we apply the Bonferroni 
correction \citep{dunn1961multiple}. This involves dividing our target significance 
($\alpha = 0.05$) by the number of comparisons ($m = 4$), resulting in a more 
conservative adjusted threshold of $\alpha_{adj} = 0.0125$.
...
MapOrderBook yields a t-statistic of t=-75.09 (df = 43.87, p<0.0001). This 
value indicates that the "signal" of the array implementation's superiority is 
75 times stronger than the measurement "noise." A similarly extreme separation 
is observed in the \texttt{dense\_full} scenario against the map alternative 
(t=-119.14, df = 30.55, p<0.0001). Even in the highest-performing non-array tier 
(\texttt{tight\_spread}), the array implementation maintains a clear statistical 
lead (t=-44.92, p < 0.0001). This formal analysis confirms that the memory-layout 
optimisation in the array implementation delivers a genuine, statistically 
significant latency reduction that cannot be attributed to environment jitter.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrr}
        ...
        \midrule
        ArrayOrderBook   & 1,442,735 &   302,223 \\
        MapOrderBook     &   799,534 &   299,044 \\
        VectorOrderBook  &   693,564 &   201,457 \\
        HybridOrderBook  &   717,375 &   283,384 \\
        PoolOrderBook    &   810,382 &   297,033 \\
        \bottomrule
    \end{tabular}
    \caption{Mean Throughput Across Implementations in Direct Mode (7 scenarios) and Gateway Mode (3 scenarios: \texttt{tight\_spread}, \texttt{mixed}, \texttt{dense\_full})}
...
spread across all implementations reached 51.9\% (1,442,735 vs 693,564~ops/s).
In gateway mode (for the three available scenarios), the spread is 33.3\%
(302,223 vs 201,457~ops/s), with non-vector implementations still clustered
more tightly than direct mode.
...
high-fidelity replicates for the most significant market regimes. Given that 
each gateway run requires full-stack network and queue processing, this focused 
selection balances campaign duration with the statistical precision required for H3.
...
the primary non-vector books (array/map/pool; see Table~\ref{tab:gateway_decomposition}),
in accordance with~\NFR{4}. The data shows that the ordering structure of the 
engine becomes largely irrelevant when the transport stack dominates the pipeline.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{Compact gateway decomposition evidence for primary non-vector books (derived from Table~\ref{tab:gateway_decomposition})}
    \label{tab:gateway_share_compact}
\end{table}

Table~\ref{tab:gateway_share_compact} clarifies the masking effect:
for the primary non-vector books (array, map, pool), the network alone consumes
88--96\% of total end-to-end latency, while engine compute remains near zero
as a percentage share.

To make interval evidence explicit for key direct comparisons, Table~\ref{tab:direct_key_latency_ci}
reports 95\% confidence intervals for representative shallow, mixed, and dense
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llrr}
        \toprule
        \textbf{Scenario} & \textbf{Implementation} & \textbf{n} & \textbf{Mean $\pm$ 95\% CI (ns)} \\
        \midrule
        \texttt{tight\_spread} & ArrayOrderBook  & 30 & 216.33 $\pm$ 13.18 \\
        \texttt{tight\_spread} & MapOrderBook    & 30 & 639.42 $\pm$ 14.05 \\
        \texttt{tight\_spread} & PoolOrderBook   & 30 & 596.33 $\pm$ 16.22 \\
        \texttt{tight\_spread} & HybridOrderBook & 30 & 678.38 $\pm$ 10.14 \\
        \midrule
        \texttt{mixed} & ArrayOrderBook  & 30 & 209.76 $\pm$ 5.50 \\
        \texttt{mixed} & MapOrderBook    & 30 & 644.26 $\pm$ 10.48 \\
        \texttt{mixed} & PoolOrderBook   & 30 & 601.28 $\pm$ 12.70 \\
        \texttt{mixed} & HybridOrderBook & 30 & 690.22 $\pm$ 21.84 \\
        \midrule
        \texttt{dense\_full} & ArrayOrderBook  & 30 & 206.51 $\pm$ 2.47 \\
        \texttt{dense\_full} & MapOrderBook    & 30 & 1,099.85 $\pm$ 15.13 \\
        \texttt{dense\_full} & PoolOrderBook   & 30 & 1,224.14 $\pm$ 66.74 \\
        \texttt{dense\_full} & HybridOrderBook & 30 & 2,143.29 $\pm$ 58.57 \\
        \bottomrule
    \end{tabular}
    \caption{Representative direct-mode latency interval evidence (95\% CI)}
...
order book structures is consistent with mechanical sympathy~\citep{fowler2011lmax}. Because elements
...
higher dispersion in branch-heavy mixes. As shown in 
Figure~\ref{fig:latency_stddev_no_vector_heatmap}, the Hybrid implementation 
exhibits significantly higher dispersion in branch-heavy workloads. In the raw 
...
confidence intervals in the mixed scenario (Pool: [589.30, 613.25] ns vs. 
Map: [634.39, 654.13] ns). Using Welch's t-test to account for 
unequal variances, this separation yields a decisive result 
(t=-6.54, p<0.0001), formally proving that pooling delivers 
a genuine latency reduction over the standard map. However, 
this advantage remains marginal compared to the ArrayOrderBook 
([204.58, 214.95] ns), which supports the latter half of H2: 
optimising allocation logic does not resolve the fundamental 
sequential bottleneck of the underlying search structure. 
The book also loses its lead in dense\_full (1,224.14 ns vs 1,099.85 ns 
for map) because the increased branching penalty outweighs 
the gains from memory locality.
...
array (1,152~ns vs 246~ns) due to the high frequency of branch mispredictions 
...
to map (3,096~ns vs 3,158~ns for map). 
...
comparisons are supported by separated 95\% intervals, which strengthens the
...
non-overlapping 95\% confidence intervals (separation of $\approx$400 ns, 
p < 0.0001). However, in Gateway mode, the end-to-end 95\% intervals 
for the same pair (Array: [2.52, 3.07] ms; Map: [2.45, 2.77] ms) exhibit 
a significant overlap of over 220,000 ns. When subjected to 
Welch's t-test, this comparison yields an insignificant result 
(t= 1.28, p= 0.21). This transition from a p < 0.0001 result to a 
...
\texttt{tight\_spread} and \texttt{mixed}, but they do not fully converge in
...
This throughput degradation is statistically definitive; the 95\% confidence 
intervals for the 1-producer configuration ([3,830,000, 4,010,371] ops/s) and 
the 8-producer configuration ([3,246,000, 3,415,161] ops/s) are strictly 
non-overlapping. A Welch's t-test confirms this significant decline 
(t = 12.44, p < 0.0001), proving that the increase in producer threads 
creates a systemic bottleneck rather than providing parallel speedup. 
This "reverse scaling" suggests that adding producer threads increases 
contention on the shared Multi-Producer Single-Consumer (MPSC) queue 
and exacerbates L3 cache coherence effects.
...
for H3. Even as the transport layer (the queue) degrades under contention, 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.32\textwidth}p{0.62\textwidth}}
        ...
        Queue-to-engine dominance (array) & Queue wait is about 13,910x--20,987x larger than engine time \\
        \bottomrule
    \end{tabular}
...
Table~\ref{tab:mpsc_compact_evidence} summarizes the same contention result in
...
tracking (\FR{3}) were met using the triple-timer design. 
...
system-level masking effect expected under~\NFR{4}.
...
\begin{itemize}
    ...
    \item \textbf{Statistical Sample Size.}
        ...
        mode and $n=20$ in gateway mode). This supports confidence intervals for
