...
To manage the five discrete order book implementations, we employed a 
unified interface and run-time polymorphism. This allowed us to maintain 
a modular architecture while ensuring identical compiler flags across 
all variants. 
...
\begin{itemize}
    ...
    \item \textbf{POSIX Socket API}: We opted for the raw \texttt{sys/socket.h} API over higher-level networking 
libraries. 
By bypassing those abstractions, we ensure the gateway's latency is purely a 
function of the OS kernel stack traversal.
    \item \textbf{Functional Parity Testing}: To ensure that every order book variant produces identical matching results, 
we built a unit testing suite around the \texttt{GoogleTest} framework. 
This validates that implementations remain functionally consistent, even 
...
levels directly to a contiguous array. 
While this approach is the fastest possible method for price lookups, it 
introduces a practical constraint: the book must know its price range at 
startup. 
We chose this trade-off to establish a theoretical performance floor for the 
study, fulfilling the requirements for low-latency execution (\FR{2}). 
Mapping prices to indices, as shown in the listing below, is a simple arithmetic
operation that remains stable regardless of the book's depth.
...
void ArrayOrderBook::addOrder(const Order &order) {
    ...
    if (order.side == Side::Buy) {
        bidLevels_[index].push_back(order);
        activeBidLevels_[index] = true;
        if (order.price > cachedBestBid_) cachedBestBid_ = order.price; // O(1)
    } else {
        askLevels_[index].push_back(order);
        activeAskLevels_[index] = true;
        if (order.price < cachedBestAsk_) cachedBestAsk_ = order.price; // O(1)
    }
}
\end{lstlisting}
...
is analysed in Section~\ref{subsec:pool_hybrid_mech}.

\subsection{Pool-Allocated Order Book (\hyperref[app:glossary]{Slab Allocation})}
...
\begin{lstlisting}[language=C++, caption={Slab-Allocation and Free List Management}, label={lst:slab_allocation}]
...
analysed in Section~\ref{subsec:pool_hybrid_mech}.

\section{The TCP Order Gateway}
To evaluate the system under realistic network conditions for \ref{hyp:h3}, we implemented 
...
parser that processes raw buffers as they arrive. As the listing below 
...
\begin{lstlisting}[language=C++, caption={Incremental FIX Parsing Loop}, label={lst:fix_parsing}]
...
To measure kernel networking overhead without the jitter of hardware 
NICs, we run the gateway on the loopback interface (\texttt{127.0.0.1}). 
This allows us to isolate the "Wire-to-Engine" latency analysed in 
Section~\ref{sec:gateway_decomposition}.
...
with \texttt{std::memory\_order\_acquire/release} semantics~\citep{michael1996simple}, ensuring 
...
reflect the underlying data structure performance rather than our own 
...
Counter (\hyperref[app:glossary]{\texttt{RDTSC}}), our implementation uses the standard C++ library 
...
\begin{lstlisting}[language=C++, caption={Nanosecond Precision Timing}, label={lst:rdtsc}]
// src/utils/rdtsc.hpp
...
we implemented CPU pinning as shown in Listing~\ref{lst:cpu_pinning}.

\begin{lstlisting}[language=C++, caption={POSIX Thread Pinning Strategy}, label={lst:cpu_pinning}]
...
thread between CPUs. 
This ensures that the engine's working set remains "hot" in the local L1/L2 
caches, maintaining the deterministic execution time required to validate 
our performance claims. 
This pinning helps satisfy \NFR{4} by making sure our measurements are not 
skewed by operating system interference.

\section{Automation and Data Collection}
The suite uses a Python-based driver to sweep across the benchmark 
scenarios defined in Chapter~\ref{chap:system_design}. Each benchmark run 
was conducted with a base volume of 10,000 orders to ensure statistical 
stability. To prevent "cold-cache" artefacts, every measurement run began 
with a 1,000-order warmup phase (consistent with benchmark driver logic) 
where the memory was touched and the branch predictor trained before 
recordings commenced. 
This driver coordinates the execution of each order book variant and manages 
the serialised CSV output required for the detailed evaluation in 
Chapter~\ref{chap:testing_evaluation}.
