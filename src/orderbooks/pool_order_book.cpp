...
namespace hft
{
    ...
    void PoolOrderBook::appendToLevel(MapType &ladder, Index idx, Price price)
    {
    ...
    template <typename MapType>
    void PoolOrderBook::removeFromLevel(MapType &ladder, Index idx)
    {
        Price price = orders_[idx].order.price;
        auto levelIt = ladder.find(price);
        
        if (levelIt == ladder.end()) return;
